...
def comp(update: Update, context: CallbackContext) -> int:
    ...
    if status == 100:
        ...
        if plan == 1:
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            query.edit_message_text(
                text="<b>¡Pago recibido!✅.\n<i>Tu membresía está activa por 1 mes 🎉</i></b>\n\nPresiona el botón <i>'Ver links🔗'</i> sólo una vez y espera a que carguen los enlaces.\n\n⚠️<i>Si te unes a los canales VIP demasiado rápido Telegram bloqueará temporalmente la función, en ese caso solo debes esperar unos minutos y volver a intentarlo</i>", parse_mode=ParseMode.HTML)
            ahora = datetime.now()
...
def activarPrueba(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        query.edit_message_text(
            text="🎉 <b>¡Felicidades! tu prueba de <u>10 días con acceso a todos los canales</u> ha sido activada!</b> 🎁\n\nPresiona el botón una vez y espera a que se carguen los links👇<i>\n(Paciencia, puede demorar unos segundos⏳)\n\n⚠️Si te unes a los canales VIP demasiado rápido Telegram bloqueará temporalmente la función, en ese caso solo debes esperar unos minutos y volver a intentarlo</i>", parse_mode=ParseMode.HTML)
        os.system('python3 pyro.py')
