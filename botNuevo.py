...
def coinPaymnets2meses(update: Update, context: CallbackContext) -> int:
    global coin_dos_meses, usdt, busd
    query = update.callback_query
    query.answer()
    keyboard = [
            [InlineKeyboardButton("USDT.TRC20 (Red de TronTRX)", callback_data=str(USDT))],        
...
def coinPaymnets3meses(update: Update, context: CallbackContext) -> int:
    global coin_tres_meses, usdt, busd
    query = update.callback_query
    query.answer()
    keyboard = [[
            InlineKeyboardButton("USDT.TRC20 (Red de TronTRX)", callback_data=str(USDT)),           
...
def main() -> None:
    ...
    updater = Updater("5365579677:AAGOa0O6QT1UvAEmzBLZvF-5bNvJkD6T1t4")#5342854757:AAGhL0uidihNAOHUadSPFKsfW587EgOdlxA")
    j = updater.job_queue
